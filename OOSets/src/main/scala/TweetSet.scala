...
class Empty extends TweetSet {
  ...
  def union(set2: TweetSet): TweetSet = set2

  def mostRetweeted: Tweet = throw new NoSuchElementException("Enpty set")
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def union(set2: TweetSet): TweetSet = this.filterAcc(tweet => true, set2)
  ...
  def mostRetweeted: Tweet = {
    if (left.isEmpty && right.isEmpty)
      ...
      max(max(left.mostRetweeted, right.mostRetweeted), elem)
  }
